%% Parameter estimation of the possible model (4) Regulation by DP to cTEC
% presented in Fig. 5h
...
ps_name = {
    ...
    'p_ctec' 'omega_ctec' 'nctec_0' 'phi_c' 'delta_c' 'mu_c' 'gamma_c'...    
...
filename = 'params/dp_to_ctec.mat';
load(filename);
...
dde_model = @dde_model_dp_to_ctec;
[ps_opt,resnorm,residual,exitflag,output,lambda,jacobian] = ...
    lsqnonlin(@(ps) diff_model_handle(dde_model, ps, data_xray, ps_order, ps_name, lag_names), ps_give, lb, ub, opts);
...
sol = dde23(@(t, n, Z) dde_model_dp_to_ctec(t, n, Z, params), lags, @(t) dde_history_coarse_grained(t, params),[0, 49]);
